 ...
 * Equalizer class
 *
 * Simple class to keep band amplification values and create the
...
public:
    ...
    /**
     * Set band amplification factor.  
     * @param idx Band index
     * @param value Band value.  It should be between 0 and 2
     */
    ...
    /**
     * Return the last set frequency response
     */
...
protected:
    ...
    /**
     * Linearly interpolate filter frequency response from the given points,
     * @param data array of floats where the final data will be stored
     */
    void interpolate(float* data);

    /**
     * The impulse response will be computed out of the given magnitude
     * response of H(w)
     */
